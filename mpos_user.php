...
if (isset($_POST['save_user'])) {
    ...
    // CEK UNIQUE
    if ($id) {
        $stmt = $conn->prepare("SELECT * FROM mpos WHERE (outlet_id=? OR sn_mesin=?) AND id<>?");
        $stmt->bind_param("ssi", $outlet_id, $sn_mesin, $id);
    } else {
        $stmt = $conn->prepare("SELECT * FROM mpos WHERE outlet_id=? OR sn_mesin=?");
    ...
    if ($res->num_rows > 0) {
        $msg = "Outlet ID atau SN Mesin sudah ada!";
    } else {
        if ($id) {
            // EDIT
        ...
        if ($stmt2->execute()) {
            $status_stok = '';
            if ($pengajuan == 'IMPLEMENTASI') {
                $status_stok = 'terpakai';
            } elseif ($pengajuan == 'REPLACE' || $pengajuan == 'RETURN') {
            ...
            $msg = $id ? "Data berhasil diupdate & stok diperbarui!" : "Data berhasil disimpan & stok diperbarui!";
        } else {
